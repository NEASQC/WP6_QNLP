...
class Alpha4(torch.nn.Module):
    ...
    def forward(self, sentence_tensors : torch.tensor)-> None:
        ...
        2 ** n_measured_qubits. If this size is equal to the number of classes, 
        we will take these vectors as outputs.
        If our number of classes is different from 2 ** n_measured_qubits, 
        (which again is the size of the output probabilities tensor of our
    ...
    def compute_probs(self, sentence_tensors : torch.tensor)-> None:
        ...
        containing a number of sentence vectors equal to the 
        batch size. If the n_classes it's equal to
        2 ** (number of measured qubits) then the model output 
        can be taken, as the output represents probability values
        which are already normalised. When n_classes it's not equal
        to the aforementioned quantity, then a softmax layer needs to 
        be applied to normalise the outputs of the model (as we 
        are just taking probabilities output from the circuit up
    ...
    def compute_preds(self, sentence_tensors : torch.tensor)-> None:
        """
        Compute the output predictions of the network for a tensor 
