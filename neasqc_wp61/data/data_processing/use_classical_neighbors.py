...
def main():
    ...
    name_file = args.output + f"classical_neighbors_{args.k}_{args.runs}"
    ...
    for i in range(args.runs):
        ...
        with open(name_file + f'classical_predictions_run_{i}.pickle', 'wb') as file:
    ...
    with open(name_file + "classical_predictions.txt", "w") as output:
        for pred in predictions_majority_vote:
            output.write(f"{pred}\n")
    for i in range(args.runs):
        os.remove(name_file + f'classical_predictions_run_{i}.pickle')
