...
def plot_experiment_results(json_file):
    ...
    if data.get('train_loss') is not None and data.get('val_loss') is not None:
        ...
        interactive_plotly_plot(x_values, mean_train_losses, std_train_losses, 1, col_index, "Training Loss", fig)
        # Add scatter validation loss trace
        interactive_plotly_plot(x_values, mean_val_losses, std_val_losses, 2, col_index, "Validation Loss", fig)
        col_index += 1


    if data.get('train_acc') is not None and data.get('val_acc') is not None:
        # Add scatter training accuracy trace
        interactive_plotly_plot(x_values, mean_train_accuracies, std_train_accuracies, 1, col_index, "Training Accuracy", fig)
        # Add scatter validation accuracy trace
        interactive_plotly_plot(x_values, mean_val_accuracies, std_val_accuracies, 2, col_index, "Validation Accuracy", fig)
...
def interactive_plotly_plot(x_values, y_values, y_std, row_index, col_index, name, fig):
    # Define a colormap (you can choose any colormap of your preference)
    colormap = plt.get_cmap('viridis')

    fig.add_trace(
        go.Scatter(x=x_values,y=y_values, mode='lines', showlegend=False, name=name),
        row=row_index, col=col_index
    )
    fig.add_trace(
        go.Scatter(x=x_values,y=y_values+y_std, mode='lines',line=dict(width=0), showlegend=False, name='Upper Bound'),
        row=row_index, col=col_index
    )
    fig.add_trace(
        go.Scatter(x=x_values,y=y_values-y_std, mode='lines',line=dict(width=0),
        fill='tonexty', showlegend=False, name='Lower Bound'),
        row=row_index, col=col_index
    )
