...
The main scope of the pre-alpha model is to build a variational quantum algorithm to make sentence classification depending on if they are True or False. The structure of the analyzed sentences will be: 
...
After the models are trained we compute and compare their accuracy on train and test dataset. 

## Variational quantum circuit classfier

A variational quantum circuit is built with initial random parameters. In the circuit, only one qubit is measured (post-selection), and the variational qubits will be optimized using the labeled sentences in dataset so that the state obtained when measuring the qubit coincides with the value of the sentence (FALSE=0, TRUE=1). 

After the variational circuit is trained the accuracy over the train and test dataset will be measured and compared. 
