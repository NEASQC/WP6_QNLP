...
sys.path.append(current_path + "/../neasqc_wp61/models/quantum/alpha_4/")
import circuit as circ
from alpha_4 import Alpha4 as alpha_4
    ...
    for optimiser in optimisers_list:
        for ansatz in ansatze_list:
            for init in initialisations_list:
                ...
                observables = {
                    key : random.choice(
                        observables_list) for key in obsevables_circuit_keys
                }
                axis_embedding = random.choice(axis_embedding_list)
                circuit = ansatz(
                    n_qubits_circuit, n_layers, axis_embedding,
                ...
                alpha_4_model = alpha_4(
                    vectors, labels_one_hot_encoding, n_classes,
                    circuit, optimiser, epochs, batch_size, 
                    torch.nn.CrossEntropyLoss, {'lr' : lr}, 
                    'cpu', test_args['seed'], init
                )
                alpha_4_model.train()
                ...
                params_run.extend(
                    ...
                    vectors, labels_one_hot_encoding, n_classes,
                    circuit, optimiser, epochs, batch_size,
                    lr, test_args['seed'], init, vector_length,
                    ansatz, n_layers, axis_embedding, observables,
                    loss_train, loss_val, preds_train, preds_val,
                    preds_test, probs_train, probs_val, probs_test
                    ]
                )
...
names_parameters = (
    'alpha_4_model', 'vectors', 'labels_one_hot_encoding', 'n_classes',
    'circuit', 'optimiser', 'epochs', 'batch_size', 'lr', 'seed',
    'init_qubits', 'vector_length', 'ansatz', 'n_layers',
    'axis_embedding', 'observables', 'loss_train', 'loss_val', 
    'preds_train', 'preds_val', 'preds_test', 'probs_train',
    'probs_val', 'probs_test'
)

@parameterized_class(
    names_parameters, set_up_test_parameters(test_args)
)
class TestAlpha4(unittest.TestCase):
    def test_model_raises_errors_if_number_of_qubits_is_not_correct(
        ...
        circuit = self.ansatz(
            self.vector_length - 1, self.n_layers, self.axis_embedding,
            observables, output_probabilities = False)
        with self.assertRaises(ValueError):
            alpha_4_model = alpha_4(
                self.vectors, self.labels_one_hot_encoding, self.n_classes,
                circuit, self.optimiser, self.epochs, self.batch_size, 
                torch.nn.CrossEntropyLoss, {'lr' : self.lr}, 'cpu',
                self.seed, self.init_qubits
            )
    ...
    def test_model_raises_error_if_out_probabilities_false_in_circuit(
        ...
        """
        circuit = self.ansatz(
            self.vector_length, self.n_layers, self.axis_embedding,
            self.observables, output_probabilities = False
        )
        with self.assertRaises(ValueError):
            alpha_4_model = alpha_4(
                self.vectors, self.labels_one_hot_encoding, self.n_classes,
    ...
    def test_model_raises_error_if_not_enough_observables(
        self
    )-> None:
        """
    ...
    def test_number_of_preds_is_correct(self)-> None:
        ...
        for i,preds in enumerate(
            [self.preds_train, self.preds_val]
        ):
            self.assertEqual(
                len(preds), self.epochs
            )
            for j in range(len(preds)):
                self.assertEqual(
                    len(preds[j]), len(self.vectors[i])
                )
    ...
    def test_number_of_probs_is_correct(self)-> None:
        ...
        for i,probs in enumerate(
            [self.probs_train, self.probs_val]
        ):
            self.assertEqual(
                len(probs), self.epochs
            )
            for j in range(len(probs)):
                self.assertEqual(
                    len(probs[j]), len(self.vectors[i])
                )
